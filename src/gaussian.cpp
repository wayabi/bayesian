...
std::shared_ptr<matrix> matrix::inv() const
{
	vec v;
	...
	return make_shared<matrix>(v, dimension_);
}
...
double gaussian::get_y(const vec& p)
{
	...
	double y = a * exp(-0.5 * matrix::dot(inv->head_mul(sub), sub));
	return y;
}
...
double gaussian::calc_percentile(const vec& p) const
{
	...
	double x = -0.5 * matrix::dot(inv->head_mul(sub), sub);
	cout << "x:" << x << endl;
	int idx = (int)(-100 * x);
	double per = 0;
	if(idx >= 500){
